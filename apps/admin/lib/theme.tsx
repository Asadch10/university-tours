...
/** Separate key from the public site so an admin's console preference is independent. */
export const THEME_STORAGE_KEY = 'ucpt-admin-theme';

/**
 * Runs before first paint, injected into <head> in app/layout.tsx.
 *
 * Dark is the default, so it only ever ADDS the `.light` class — the server HTML and a
 * no-JS render are already correct for the common case. Without this executing ahead of
 * paint, a light-theme admin sees a dark flash on every navigation.
 */
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  // Start on the default; the effect reconciles with what the pre-paint script applied,
  ...
  // Keep multiple console tabs in step.
  useEffect(() => {
